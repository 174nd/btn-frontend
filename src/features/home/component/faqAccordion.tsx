...
export function FaQAccordion({ data }: { data: { title: string; content: string }[] }) {
  const [activeIndex, setActiveIndex] = useState<number | null>(0);

  return (
    <div className="w-full pt-8 md:px-4">
      <div className="divide-primary-200 flex w-full flex-col divide-y-3 overflow-hidden rounded-xl">
        {data.map((item, index) => (
          <div
            key={index}
